...
class LCIA():
    ...
    def dist2(self, pathway='MP', save=False, palette='GnBu'):
        """Plots the distribution of iterations for the pathway's categories."""
        ...
        for cat in lst:
            ...
            ax = plt.Subplot(fig, inner[0])
            ax.set_xticks([])
            sns.boxplot(data, ax=ax, color='goldenrod', notch=True)
            ax.set_xlabel('')
            ax.set_yticks([])
            fig.add_subplot(ax)
            i += 1
            ...
            ax = plt.Subplot(fig, inner[1])
            ax.axvline(data.mean(), 0, 1, color='darkgoldenrod', ls= '--')
            sns.distplot(data, kde=False, color='peru', ax=ax)
            ax.set_yticks([])
            ax.set_xlabel(cat, fontsize=12)
            ax.ticklabel_format(axis='x', style='sci', scilimits=(0,0), useMathText=True)
            ax.set_xticks([data.min(), data.mean(), data.max()])
